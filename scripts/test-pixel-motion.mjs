...
assert.match(index, /pixel-motion\.css\?v=3/, 'Pixel motion stylesheet must be loaded');
...
assert.match(motion, /--pixel-motion-marquee:24s/, 'The header marquee must use the readable duration token');
assert.match(motion, /pixel-header-scroll var\(--pixel-motion-marquee\) linear infinite/, 'The header text must scroll continuously');
assert.match(motion, /pixel-cursor-blink 720ms steps\(1,end\) infinite/, 'The header cursor must blink with stepped timing');
