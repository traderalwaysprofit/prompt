...
test.describe('SAMSON adaptive UI system', () => {
  test('offers four UI personalities, keeps Samson Default as default, and persists selection', async ({ page }) => {
    ...
    await expect(page.locator('body')).toHaveCSS('background-color', 'rgb(9, 13, 26)');
  ...
  test('Pixel personality matches the clean-room adventure visual contract', async ({ page }) => {
    await page.goto(BASE_URL, { waitUntil: 'networkidle' });
    await page.evaluate(() => window.SamsonTheme.set('pixel'));
    ...
    await expect(page.locator('.pixel-wire-status')).toHaveText('ADVENTURE UI');
    ...
    await expect(heroCard).toHaveCSS('border-top-width', '3px');
    await expect(heroCard).toHaveCSS('box-shadow', 'rgb(5, 7, 19) 5px 5px 0px 0px');

    const button = page.locator('[data-show-workflows]');
    await expect(button).toHaveCSS('border-radius', '0px');
    await expect(button).toHaveCSS('background-color', 'rgb(104, 197, 255)');
    await expect(button).toHaveCSS('color', 'rgb(5, 7, 19)');
    await expect(button).toHaveCSS('box-shadow', 'rgb(5, 7, 19) 4px 4px 0px 0px');
    ...
    await expect(page.locator('.nft-card').first()).toHaveCSS('border-top-width', '3px');
  });
